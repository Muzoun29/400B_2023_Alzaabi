...
def hernquist_mass(r,a=60*u.kpc , m_halo=1.975):
    ...
    function defines Hernquist 1990 mass profile
    ----------
    Inputs : 
        
    r : astropy quantity 
        Galactrocentric destance in kpc .
    a : astropy quantity 
        scalar raduis of Hernq.profile in kpc
    m_halo : float
        total halo mass in units of 1e12 Msun
        
    Output : 
        mass : astropy quantity 
        total mass within the given radius in msun 
...
#Compute Mass within 10000Kpc 
print(f"{hernquist_mass(10000*u.kpc):.2e}")

#compute mass within 300kpc 
...
# 
# Do this by rearranging the Jacobi Radius equation to solve for the satellite mass (Msat). 
# 

#Rj=r*(Msat/2/Mhost)**(1/3)
#Msat=9Rj/r)**(3)*2*Mhost

def jacobi_mass(rj,r,m_host):
    """
    

    function that determens the min. stelaite mass neded to maintain 
    the size of a given satelite using the Jacobi radius . calc.
    
    ----------
    Inputs : 
    rj : astropy quantity 
        jacobi radius / stellar radius of satallite in kpc 
    r : astropy quantity 
        distance of satallite from the host in kpc 
    m_host : astropy quantity 
        mass of the host galacy in Msun (within r )

    outputs : 
        m_min : astropy quantity 
        min. Satellite mass in Msun 
        
    -------
    

    """
    m_min =(rj/r)**3*2*m_host # Satellite mass from the jacobi radius calc.
    return m_min
...
sizeL=18.5*u.kpc #obseved radius of LMC Disk (Mackey 2016 )
distL=50.0*u.kpc #galactocentruc distance to the LMC 
...
print(LMC_tidalM/LMC_jacobiM) #Take ratio to compare in part a 
